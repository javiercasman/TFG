...
def duplicate_collection(self, context, col_name): #Duplica la colección activada
    ...
    new_col.objects[5].hide_set(True)
    src_col.objects[3].hide_set(True)
    src_col.objects[4].hide_set(True)
    src_col.objects[5].hide_set(True)
...
def add_candle(self, context, col_name):
    ...
    translate_value = context.scene.cursor.location - new_col.all_objects[0].location
    ...
    view3d_to_3dcursor()
    new_col.objects[3].hide_set(True) #ocultar el punto
    return col_name
...
class CANDLE_OT_dialog_box(Operator):
    ...
    def execute(self,context):
        ...
        video_name = (self.video_path.split("//")[-1]) #Nombre del video (con ".mp4")
        ...
        context.scene.cursor.location = loc + (dir * Vector((5.0, 5.0, 5.0))) #no funciona bien
